...
export default function Header() {
  ...
  const [embeddedInWorkbench] = useState(isEmbeddedInWorkbench)
        ...
        <div className="safe-area-x safe-header-inner max-w-7xl mx-auto flex items-center justify-between relative">
          ...
          {embeddedInWorkbench && (
            <div className="hidden sm:flex items-center gap-1 mr-2">
              <button
                type="button"
                onClick={() => postWorkbenchAction('image-workbench:clear-state')}
                className="px-3 py-1.5 rounded-lg text-sm font-medium text-gray-500 hover:text-gray-800 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-gray-200 dark:hover:bg-white/[0.06] transition-colors"
              >
                清空页面
              </button>
            </div>
