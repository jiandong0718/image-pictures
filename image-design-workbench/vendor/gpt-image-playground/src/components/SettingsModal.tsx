...
export default function SettingsModal() {
  ...
  const [workbenchManagedApiConfig] = useState(isEmbeddedInWorkbench)
  ...
  const activeProviderShowsApiUrlConfig = activeProviderUsesApiUrl && activeProfile.provider !== 'openai'
  const activeApiUrlLocked = isApiUrlLockedForProvider(activeProfile.provider)
      ...
      >
        ...
        <div className="flex flex-1 min-h-0 flex-col sm:flex-row">
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-transparent relative overflow-hidden">
            ...
            {activeTab === 'api' && workbenchManagedApiConfig && (
              <div className="space-y-4">
                ...
                <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-white/[0.06] dark:bg-white/[0.02]">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <span className="text-sm font-semibold text-gray-800 dark:text-gray-100">当前生效配置</span>
                    <span className={`rounded px-2 py-1 text-xs font-medium ${activeProfile.apiKey ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-gray-100 text-gray-500 dark:bg-white/[0.08] dark:text-gray-400'}`}>
                      {activeProfile.apiKey ? '已配置' : '未配置'}
                    </span>
                  </div>
                  <div className="space-y-3 text-sm">
                    <div>
                      <div className="mb-1 text-xs text-gray-500 dark:text-gray-500">配置来源</div>
                      <div className="rounded-xl bg-gray-50 px-3 py-2 text-gray-700 dark:bg-white/[0.04] dark:text-gray-200">{activeProfile.name}</div>
                    </div>
                    <div>
                      <div className="mb-1 text-xs text-gray-500 dark:text-gray-500">API URL</div>
                      <div className="truncate rounded-xl bg-gray-50 px-3 py-2 font-mono text-xs text-gray-700 dark:bg-white/[0.04] dark:text-gray-200" title={activeProfile.baseUrl}>
                        {activeProfile.baseUrl || '未配置'}
                      </div>
                    </div>
                    <div>
                      <div className="mb-1 text-xs text-gray-500 dark:text-gray-500">模型</div>
                      <div className="rounded-xl bg-gray-50 px-3 py-2 font-mono text-xs text-gray-700 dark:bg-white/[0.04] dark:text-gray-200">{activeProfile.model || DEFAULT_IMAGES_MODEL}</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {activeTab === 'api' && !workbenchManagedApiConfig && (
              ...
              {/* 3. API URL */}
              {activeProviderShowsApiUrlConfig && !activeApiUrlLocked && (
                <label className="block">
