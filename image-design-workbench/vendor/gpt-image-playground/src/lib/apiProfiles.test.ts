...
describe('locked OpenAI API URL', () => {
  it('forces OpenAI profiles to use the configured default API URL', async () => {
    vi.stubEnv('VITE_DEFAULT_API_URL', 'https://locked.example.com/v1')
    vi.stubEnv('VITE_LOCK_API_URL', 'true')
    vi.resetModules()

    const {
      getActiveApiProfile: getLockedActiveApiProfile,
      isApiUrlLockedForProvider: isLockedForProvider,
      normalizeSettings: normalizeLockedSettings,
    } = await import('./apiProfiles')

    expect(isLockedForProvider('openai')).toBe(true)
    expect(isLockedForProvider('fal')).toBe(false)
    ...
    expect(settings.baseUrl).toBe('https://locked.example.com/v1')
    expect(settings.profiles[0].baseUrl).toBe('https://locked.example.com/v1')
    expect(getLockedActiveApiProfile({
      ...settings,
      baseUrl: 'https://runtime.example.com/v1',
    }).baseUrl).toBe('https://locked.example.com/v1')
  })

  it('keeps fal profiles on their own URL when OpenAI URL locking is enabled', async () => {
    vi.stubEnv('VITE_DEFAULT_API_URL', 'https://locked.example.com/v1')
    vi.stubEnv('VITE_LOCK_API_URL', 'true')
    vi.resetModules()

    const { normalizeSettings: normalizeLockedSettings } = await import('./apiProfiles')
    const settings = normalizeLockedSettings({
      profiles: [{
        id: 'fal-profile',
        name: 'fal',
    ...
    expect(settings.profiles[0].baseUrl).toBe('https://fal-proxy.example.com')
  })
})
