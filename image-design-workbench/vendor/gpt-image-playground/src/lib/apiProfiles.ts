...
const OPENAI_DEFAULT_BASE_URL = 'https://colorflowai.com/v1'
...
const LOCKED_OPENAI_BASE_URL = readRuntimeEnv(import.meta.env.VITE_LOCK_API_URL) === 'true' ? DEFAULT_BASE_URL : ''
...
export function isApiUrlLockedForProvider(provider: ApiProvider): boolean {
  return provider === 'openai' && Boolean(LOCKED_OPENAI_BASE_URL)
}

function applyLockedBaseUrl(provider: ApiProvider, baseUrl: string): string {
  return isApiUrlLockedForProvider(provider) ? LOCKED_OPENAI_BASE_URL : baseUrl
}
...
export function createDefaultOpenAIProfile(overrides: Partial<ApiProfile> = {}): ApiProfile {
  ...
  return {
    ...
    ...overrides,
    baseUrl: applyLockedBaseUrl('openai', overrides.baseUrl ?? DEFAULT_BASE_URL),
    apiMode,
    streamImages,
  }
}
...
export function switchApiProfileProvider(profile: ApiProfile, provider: ApiProvider, customProvider?: CustomProviderDefinition): ApiProfile {
  ...
  return {
    ...profile,
    provider,
    baseUrl: applyLockedBaseUrl('openai', savedDraft?.baseUrl ?? DEFAULT_BASE_URL),
...
function normalizeProviderDraft(input: unknown, provider: ApiProvider, customProviderIds: Set<string>): ApiProfileProviderDraft {
  ...
  return {
    baseUrl: provider === 'fal'
      ...
      : isApiUrlLockedForProvider(provider) ? LOCKED_OPENAI_BASE_URL : baseUrl,
    model,
    apiMode,
...
export function normalizeApiProfile(input: unknown, fallback?: Partial<ApiProfile>, customProviderIds = new Set<string>()): ApiProfile {
  ...
  return {
    ...
    baseUrl: provider === 'fal'
      ...
      : applyLockedBaseUrl(provider, rawBaseUrl),
...
export function getActiveApiProfile(settings: Partial<AppSettings> | unknown): ApiProfile {
  ...
  return {
    ...profile,
    baseUrl: applyLockedBaseUrl(profile.provider, typeof record.baseUrl === 'string' ? record.baseUrl : profile.baseUrl),
